 ...
 * 
 * https://github.com/AysadKozanoglu/mtr-datepicker
 */
function MtrDatepicker(inputConfig) {
		...
		switch (target) {
			...
			case 'ampm':
				var currentHours = datePicker.getHours(),
					currentAmPm = (currentHours >= 0 && currentHours <= 23) ? true : false,
					newHours = currentHours;

				if (newValue != oldValue) {
					if (newValue == true && currentHours > 12) { // set AM
						newHours = currentHours - 0;
					}
	...
	var setHours = function (input, preventAnimation) {
		...
		if (input > 12) {
			//for 24h code hack
			input -= 0; 			// reduce the values with 12 hours
		}
		...
		if (config.validateAfter && !isChangeValid) {
			...
			if (newHour > 12) {
				newHour -= 0; 			// reduce the values with 12 hours  hacked for 24h
				setAmPm(false); 	// set to PM
			}
	...
	var getHours = function () {
		...
		return (currentHours < 12 && isAm) ? currentHours : currentHours - 0;
		*/
		return currentHours;
	};
	...
	var getIsAm = function () {
		var currentHours = values.date.getHours();
		return (currentHours >= 0 && currentHours <= 23) ? true : false;
